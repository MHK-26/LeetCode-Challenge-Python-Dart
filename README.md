# LeetCode Solutions

This repository contains my solutions to LeetCode problems using Dart and Python programming languages.


## Solutions

### Problem 1: Contains Duplicate (Easy)
- [Dart Solution](./dart/contains_duplicate.dart)
- [Python Solution](./python/contains_duplicate.py)

### Problem 2: Valid Anagram (Easy)
- [Dart Solution](./dart/valid_anagram.dart)
- [Python Solution](./python/valid_anagram.py)

### Problem 3: Two Sum (Easy)
- [Dart Solution](./dart/two_sum.dart)
- [Python Solution](./python/two_sum.py)

### Problem 4: Valid Palindrome (Easy)
- [Dart Solution](./dart/valid_palindrome.dart)
- [Python Solution](./python/valid_palindrome.py)

### Problem 5: Best Time to Buy And Sell Stock (Easy)
- [Dart Solution](./dart/best_time_to_buy_and_sell_stock.dart)
- [Python Solution](./python/best_time_to_buy_and_sell_stock.py)

### Problem 6: Valid Parentheses (Easy)
- [Dart Solution](./dart/valid_parentheses.dart)
- [Python Solution](./python/valid_parentheses.py)

### Problem 7: Reverse Linked List (Easy)
- [Dart Solution](./dart/reverse_linked_list.dart)
- [Python Solution](./python/reverse_linked_list.py)

### Problem 8: Merge Two Sorted Lists (Easy)
- [Dart Solution](./dart/merge_two_sorted_linked_lists.dart)
- [Python Solution](./python/merge_two_sorted_linked_lists.py)

### Problem 9: Linked List Cycle (Easy)
- [Python Solution](./python/linked_list_cycle.py)

### Problem 10: Invert Binary Tree (Easy)
- [Dart Solution](./dart/invert_binary_tree.dart)
- [Python Solution](./python/invert_binary_tree.py)

### Problem 11: Maximum Depth of Binary Tree (Easy)
- [Dart Solution](./dart/maximum_depth_of_binary_tree.dart)
- [Python Solution](./python/maximum_depth_of_binary_tree.py)

### Problem 12: Same Tree (Easy)
- [Dart Solution](./dart/same_tree.dart)
- [Python Solution](./python/same_tree.py)

### Problem 13: Subtree of Another Tree (Easy)
- [Dart Solution](./dart/subtree_of_another_tree.dart)
- [Python Solution](./python/subtree_of_another_tree.py)

### Problem 14: Climbing Stairs (Easy)
- [Dart Solution](./dart/climbing_stairs.dart)
- [Python Solution](./python/climbing_stairs.py)

### Problem 15: Meeting Rooms (Easy)
- [Python Solution](./python/meeting_rooms.py)

### Problem 16: Number of 1 bits (Hamming weight) (Easy)
- [Python Solution](./python/number_of_1_bits_hamming_weight.py)

### Problem 17: Counting bits (Easy)
- [Dart Solution](./dart/counting_bits.dart)
- [Python Solution](./python/counting_bits.py)

### Problem 18: Reverse bits (Easy)
- [Python Solution](./python/reverse_bits.py)

### Problem 19: Missing Number (Easy)
- [Python Solution](./python/missing_number.py) (not done)

### Problem 20: Group Anagrams (Medium)
- [Python Solution](./python/group_anagram.py)

### Problem 21: Top K Frequent Elements (Medium)
- [Python Solution](./python/top_K_frequent_elements.py)

